...
function StoreDeliveries() {
  ...
  const [statusFilter, setStatusFilter] = useState('PREPARATION') // Prêtes à être livrées
  ...
  const loadOrders = async () => {
    try {
      ...
      let list = (res?.orders ?? []).filter(
        (o) => o.status === 'PREPARATION' || o.status === 'LIVRAISON'
      )
      if (activeStoreId) list = list.filter((o) => o.shopId === activeStoreId)
      setOrders(list)
    } catch (error) {
  ...
  const handleStartDelivery = async (order) => {
    try {
      await updateOrder(order.id, { status: 'LIVRAISON' })
  ...
  const handleCancelDelivery = async (order) => {
    try {
      await updateOrder(order.id, { status: 'PREPARATION' })
  ...
  const filteredOrders = orders.filter((order) => {
    ...
    return (
      ...
      order.shopId?.toLowerCase().includes(search)
    )
  })

  // Statistiques
  const stats = {
    ready: orders.filter((o) => o.status === 'PREPARATION').length,
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (value) => (
        ...
        >
          {value === 'PREPARATION' ? 'Prête' : value === 'LIVRAISON' ? 'En livraison' : value}
        </span>
      ),
    },
    {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="relative">
            ...
            >
              ...
              <option value="PREPARATION">Prêtes à livrer</option>
