...
import { isOrderTimeAllowed } from '../../utils/orderTimeValidation'
...
function OrderCreate() {
  ...
  useEffect(() => {
    if (stores.length && !selectedStoreId) selectStore(stores[0].id)
  ...
  const handleSendOrder = async () => {
    // ✅ VALIDATION HORAIRE : Vérifier si dans la plage 12h-20h
    const timeCheck = isOrderTimeAllowed()
    if (!timeCheck.allowed) {
      showError(timeCheck.message)
      return
    }
  ...
  return (
    <ProtectedRoute requiredRole="CLIENT" requiredResource={RESOURCES.ORDERS} requiredAction={ACTIONS.CREATE}>
      <div className="space-y-6 animate-fade-in">
        ...
        {/* ✅ ALERTE HORAIRES : Afficher statut ouvert/fermé (12h-20h) */}
        <OrderTimeAlert role="CLIENT" />
