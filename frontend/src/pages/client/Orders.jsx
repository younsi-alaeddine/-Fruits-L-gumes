...
function ClientOrders() {
  ...
  useEffect(() => {
    ...
    if (storeFromUrl) {
      ...
      localStorage.setItem('selectedStoreId', storeFromUrl)
    }
  }, [searchParams])
  ...
  const loadOrders = async () => {
    try {
      ...
      let list = res?.orders ?? []
      if (selectedStoreId) list = list.filter((o) => o.shopId === selectedStoreId)
      const total = list.length
      const pageSize = 20
      const totalPages = Math.max(1, Math.ceil(total / pageSize))
      const start = (currentPage - 1) * pageSize
      setOrders(list.slice(start, start + pageSize))
