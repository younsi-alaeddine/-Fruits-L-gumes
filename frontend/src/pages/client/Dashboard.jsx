...
function ClientDashboard() {
  ...
  const loadStats = async () => {
    try {
      setLoading(true)
      const ordersParams = { scope: 'org', limit: 500 }
      const [ordersRes, storesRes] = await Promise.all([
        getOrders(ordersParams),
        getClientShops({ page: 1, limit: 200 }),
      ])

      let orders = ordersRes?.orders || []
      const clientStores = storesRes?.shops || []
      if (selectedStoreId) orders = orders.filter((o) => o.shopId === selectedStoreId)
