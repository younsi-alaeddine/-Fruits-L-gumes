...
const Profile = () => {
  ...
  useEffect(() => {
    if (user) {
      const activeShop = user.shop || (user.shops && user.shops.length > 0 ? user.shops[0] : null);
      setFormData({
        name: user.name || '',
        email: user.email || '',
        phone: user.phone || '',
  ...
  const handleUpdateProfile = async (e) => {
    ...
    try {
      ...
      if (user.role === 'CLIENT' && activeShopId) {
        // Mettre à jour le magasin et l'utilisateur séparément
        await Promise.all([
          api.put(`/client/shops/${activeShopId}`, {
      ...
      {activeTab === 'info' && (
        <div className="profile-form-card">
          ...
          <form onSubmit={handleUpdateProfile}>
            ...
            {user.role === 'CLIENT' && (user.shop || (user.shops && user.shops.length > 0)) && (
              <div className="form-section">
