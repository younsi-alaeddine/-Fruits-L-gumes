...
const AdminProducts = () => {
  ...
  const fetchCustomCategories = async () => {
    try {
      ...
      // Phase 2: avoid silent failures (user feedback instead of console-only)
      toast.error('Impossible de charger les catégories personnalisées');
    }
  };
  ...
  const fetchCategoriesConfig = async () => {
    try {
      ...
      // Phase 2: avoid silent failures (user feedback instead of console-only)
      toast.error('Impossible de charger la configuration des catégories');
    }
  };
  ...
  const handleDelete = async (productId) => {
    ...
    try {
      setActionBusyIds(prev => new Set(prev).add(productId)); // Phase 2: prevent double click
  ...
  return (
    <div className="admin-products">
      <div className="products-header">
        <h1 style={{ flex:1 }}>Gestion des Produits</h1>
        <button
              ...
              products.map(product => {
                ...
                return (
                  <tr key={product.id} className={!product.isActive ? 'inactive' : ''}>
                    ...
                    <td>{formatPercent(product.tvaRate)}</td>
                    <td>{product.unit}</td>
                    <td>
                    ...
                    <td>
                      <button
                        ...
                        onClick={async () => {
                          if (actionBusyIds.has(product.id)) return; // Phase 2: prevent double click
                          try {
                            setActionBusyIds(prev => new Set(prev).add(product.id));
                            const submitData = new FormData();
                        ...
                        disabled={actionBusyIds.has(product.id)}
